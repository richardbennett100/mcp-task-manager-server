// src/tools/set_description_tool.ts
import { McpServer } from '@modelcontextprotocol/sdk/server/mcp.js';
import { McpError, ErrorCode } from '@modelcontextprotocol/sdk/types.js';
import {
  TOOL_NAME,
  TOOL_DESCRIPTION,
  SetDescriptionParamsSchema,
  SetDescriptionArgs,
} from './set_description_params.js';
...
export const setDescriptionTool = (server: McpServer): void => {
  const processRequest = async (args: SetDescriptionArgs): Promise<{ content: { type: 'text'; text: string }[] }> => {
    ...
    try {
      ...
      const updatedItem: FullWorkItemData = await workItemService.setDescription(args.work_item_id, args.description);

      logger.info(`[${TOOL_NAME}] Successfully set description for work item ${args.work_item_id}.`);
      return {
  ...
  server.tool(TOOL_NAME, TOOL_DESCRIPTION, SetDescriptionParamsSchema.shape, processRequest);
};

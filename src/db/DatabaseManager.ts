...
export class DatabaseManager {
    ...
    private async initializeDatabaseInternal(): Promise<void> {
        ...
        try {
            ...
            if (forceSchemaRun) { // Simpler: Always run
                logger.info('[DatabaseManager] Force schema run enabled or tables missing. Initializing/Re-initializing schema...');
                ...
                const finalCheck = await client.query(`SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'work_items');`);
